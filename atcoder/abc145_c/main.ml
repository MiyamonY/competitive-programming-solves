...
module List = struct
  ...
  let sumf  = ListLabels.fold_left ~f:(+.) ~init:0.0
...
let () =
  ...
  let arr = Scanf.lines n "%f %f" Util.tuple2 |> Array.of_list in
  let perm = List.permutations (0++^n) in
  perm |> List.map
       ~f:(fun r ->
           List.zip r (List.drop 1 r)
           |> List.fold_left ~init:0.0 ~f:(fun pred (a, b) ->
                  let x0, y0 = arr.(a) in
                  let x1, y1 = arr.(b) in
                  let d = sqrt (( x0 -. x1 ) *. ( x0 -. x1 ) +. ( y0 -. y1 ) *. ( y0 -. y1 )) in
                  pred +. d))
  |> List.sumf
  |> (fun v -> v /. (float_of_int @@ List.length perm))
  |> Printf.printf "%f\n"
