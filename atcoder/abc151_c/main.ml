...
let () =
  ...
  let was = Array.make n 0 in
  let acs = Array.make n false in
  List.iter (0++^m) ~f:(fun _ ->
      let p, s = Scanf.scan "%d %s" Util.tuple2 in
      if s = "WA" then
        (if not acs.(p-1) then was.(p-1) <- succ was.(p-1))
      else
        acs.(p-1) <- true);
  let rec aux i =
    if i = n then (0,0)
    else
      let (w, a) = aux (succ i) in
      if acs.(i) then (w + was.(i), succ a)
      else (w, a) in
  let (w, a) = aux 0 in
  Printf.printf "%d %d\n" a w
