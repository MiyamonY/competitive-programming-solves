...
let () =
  ...
  let rec aux i =
    let sub = String.sub s ~pos:0 ~len:(String.length s - i) in
    if Util.even @@ String.length sub then
      let n = String.length sub in
      if String.sub sub ~pos:0 ~len:(n / 2) = String.sub sub ~pos:(n/2) ~len:(n/2) then
        n
      else aux @@ succ i
    else
      aux @@ succ i
  in
  aux 1 |> Printf.printf "%d\n"
