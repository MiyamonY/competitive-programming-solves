...
module Char = struct
  ...
  let join cs =  String.concat "" @@ List.map cs ~f:(String.make 1)
end
...
module String = struct
  ...
  let explode s =
    List.init ~len:(String.length s) ~f:(fun i -> String.get s i)
end
...
module Counter = struct
  ...
  let of_array arr =
    ...
    Array.iter arr ~f:(fun v -> add c v);
    c

  let of_list l =
    let c = make @@ List.length l in
    List.iter l ~f:(fun v -> add c v);
    c

  let to_list c = Hashtbl.to_seq c |> List.of_seq

  let max c  =
    to_list c
    |> List.sort ~cmp:(fun (_, n) (_, m) -> -(Int.compare n m))
    |> List.hd
end
...
let () =
  let h, w = Scanf.scan "%d %d" Util.tuple2 in
  let arr  = Scanf.lines h "%s" String.explode |> List.map ~f:(Array.of_list) |> Array.of_list in
  let vals = Array.make_matrix ~dimx:h ~dimy:w (-1) in
  let n = ref 1 in
  let rec aux i j n =
    vals.(i).(j) <- n;
    if 0 <= j -1 && vals.(i).(j-1) = -1 && arr.(i).(j-1) = '#' then aux i (j-1) n;
    if j+1 < w && vals.(i).(j+1) = -1 &&  arr.(i).(j+1) = '#' then aux i (j+1) n;
    if 0 <= i - 1 && vals.(i-1).(j) = -1 && arr.(i-1).(j) = '#' then aux (i-1) j n;
    if i+1 < h && vals.(i+1).(j) = -1 && arr.(i+1).(j) = '#' then aux (i+1) j n;
  in
  let c = Counter.make (h*w) in
  Array.iteri arr ~f:(fun i lines ->
      Array.iteri lines ~f:(fun j v -> (
            if vals.(i).(j) = -1 then
              if v = '#' then
                (aux i j !n; incr n)
              else vals.(i).(j) <- 0)));
  Array.iter vals ~f:(fun line ->
      Array.iter line ~f:(fun v -> Counter.add c v));
  if Counter.to_list c |> List.exists ~f:(fun (v, c) -> v <> 0 && c = 1) then
    Printf.printf "No\n"
