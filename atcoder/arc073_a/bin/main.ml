...
let () =
  let _, t = Scanf.scan "%d %d" Util.tuple2 in
  let ls = Scanf.list " " int_of_string  in
  let rec aux pred total = function
    | [] -> total
    | s::ls ->
       if s <= pred + t then
         aux s (total - (pred + t - s) + t) ls
       else
         aux s (total+t) ls
  in
  aux (List.hd ls) t (List.tl ls) |> Printf.printf "%d\n"
