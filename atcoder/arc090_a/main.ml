...
let () =
  ...
  let ns = Scanf.array " " int_of_string |> Array.scanl ~init:0 ~f:(+) in
  let ms = Scanf.array " " int_of_string |> Array.scanl ~init:0 ~f:(+) in
  List.map (1++n) ~f:(fun i ->
      ns.(i) - ns.(0) + ms.(n) - ms.(i-1))
