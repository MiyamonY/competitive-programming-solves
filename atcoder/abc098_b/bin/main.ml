...
let () =
  ...
  List.map (0++n) ~f:(fun i ->
      let a = String.sub s ~pos:0 ~len:i |> String.explode |> CharSet.of_list in
      let b = String.sub s ~pos:i ~len:(n - i) |> String.explode |> CharSet.of_list in
      let c = CharSet.inter a b in
      CharSet.elements c |> List.length
    ) |> List.max ~cmp:Int.compare |> Printf.printf "%d\n"
