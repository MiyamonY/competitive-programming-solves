...
let () =
  ...
  let vs = Scanf.array " " int_of_string in
  let arr = Array.make (succ n) 0 in
  Array.iter vs ~f:(fun v -> arr.(v) <- succ arr.(v));
  let total = Array.fold_left arr ~init:0 ~f:(fun prev v ->
      prev + v * (pred v) / 2) in
  Array.iter vs ~f:(fun v ->
      let n = arr.(v) in
      Printf.printf "%d\n" @@ total - n * (pred n) / 2 + (pred n) * (pred @@ pred n) /2)
