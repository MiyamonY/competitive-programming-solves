...
module List = struct
  ...
  let rec transpose = function
    | []::_ -> []
    | xs -> (ListLabels.map xs ~f:List.hd) :: transpose (ListLabels.map xs ~f:List.tl)
...
let () =
  ...
  let ls =  Scanf.lines h "%s" String.explode in
  let rows = List.map ls ~f:(fun l ->
      if List.for_all l ~f:((=) '.') then
        true
      else false) in
  let cols = List.map (List.transpose ls) ~f:(fun l ->
      if List.for_all l ~f:((=) '.') then
        true
      else false) in
  List.iteri ls ~f:(fun i line ->
      List.iteri line ~f:(fun j c ->
          (if List.nth rows i then ()
          else if List.nth cols j then ()
          else Printf.printf "%c" c);
          if j = w -1 then Printf.printf "\n"
        )
    )
