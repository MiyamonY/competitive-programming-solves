...
let () =
  let n, m = Scanf.scan "%d %d" Util.tuple2 in
  let ns = Scanf.lines n "%d %d" Util.tuple2 in
  let ms = Scanf.lines m "%d %d" Util.tuple2 in
  List.iter ns ~f:(fun (x1, y1) ->
      let rec aux i min d = function
        | [] -> Printf.printf "%d\n" @@ succ min
        | (x2, y2):: ls->
           let e = abs (x1 - x2) + abs (y1 - y2) in
           if e < d then aux (succ i) i e ls
           else aux (succ i) min d ls in
      aux 0 0 Int.max_int ms)
