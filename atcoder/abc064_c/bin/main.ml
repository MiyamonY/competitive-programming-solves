...
let () =
  let _ = Scanf.scan "%d" Fun.id in
  let arr = Array.make 8 false in
  let m = ref 0 in
  Scanf.array " " int_of_string
  |> Array.iter ~f:(fun v ->
          if v < 400 then arr.(0) <- true
          else if v < 800 then arr.(1) <- true
          else if v < 1200 then arr.(2) <- true
          else if v < 1600 then arr.(3) <- true
          else if v < 2000 then arr.(4) <- true
          else if v < 2400 then arr.(5) <- true
          else if v < 2800 then arr.(6) <- true
          else if v < 3200 then arr.(7) <- true
          else incr m
       );
  let n = Array.to_list arr |> List.filter ~f:((=) true)  |> List.length in
  Printf.printf "%d %d\n" (if !m > 0 && n = 0 then 1 else n) (n + !m)
