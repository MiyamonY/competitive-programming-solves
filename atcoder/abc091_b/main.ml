...
let () =
  ...
  let ss = Scanf.lines n "%s" Fun.id in
  let m = Scanf.scan "%d" Fun.id in
  let ts = Scanf.lines m "%s" Fun.id in
  let c1 = Counter.of_list ss in
  let c2 = Counter.of_list ts in
  0::List.map (Counter.to_list c1) ~f:(fun (k, v) -> v - Counter.get c2 k)
