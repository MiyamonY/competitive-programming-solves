...
let () =
  ...
  let ss = Scanf.lines n "%d" Fun.id in
  let arr = Array.make (n * 100+1)  false in
  arr.(0) <- true;
  List.iter ss ~f:(fun s ->
      List.iter ((Array.length arr)^--0) ~f:(fun i ->
          if i - s >= 0 && arr.(i-s) then arr.(i) <- true
        )
    );
  let rec aux i  =
    if i = Array.length arr then 0
    else
      max (aux (succ i)) (if i mod 10 <> 0 && arr.(i) then i else 0)
  in
  aux 0 |> Printf.printf "%d\n"
