...
let () =
  ...
  let ls = Scanf.lines n "%d %d %d" Util.tuple3 in
  let ls = (0,0,0)::ls in
  let rec aux = function
    | [] -> true
    | ((t1, x1, y1), (t2, x2, y2))::ls->
       let d = t2 - t1 in
       let e = abs (x1 - x2) + abs (y1 - y2) in
       if e <= d && (Util.even d = Util.even e) then aux ls
       else false in
  if aux (List.zip ls (List.drop 1 ls)) then Printf.printf "Yes\n"
