...
let () =
  let s = Scanf.scan "%s" Fun.id |> String.explode |> Array.of_list in
  let k = Scanf.scan "%d" Fun.id in
  let rec aux i =
    if i >= k then '1'
    else if s.(i) <> '1' then s.(i)
    else aux @@ succ i in
  aux 0 |> Printf.printf "%c\n"
