...
let () =
  ...
  let ls = Scanf.lines n "%d" Fun.id in
  let h = Hashtbl.create n in
  List.iter ls ~f:(fun v ->
      match Hashtbl.find_opt h v with
      | None -> Hashtbl.add h v 1
      | Some n ->  Hashtbl.replace h v (succ n));
  Hashtbl.fold (fun _ n m -> if Util.odd n then succ m else m) h 0
  |> Printf.printf "%d\n"
