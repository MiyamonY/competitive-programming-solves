...
module Char = struct
  ...
  let join cs =  String.concat "" (List.map (String.make 1) cs)
end
...
let () =
  ...
  let arr = Scanf.lines h "%s" Fun.id |> Array.of_list in
  let mat = Array.make_matrix ~dimx:h ~dimy:w "" in
  List.iter (0++^h) ~f:(fun y ->
      List.iter (0++^w) ~f:(fun x ->
          if arr.(y).[x] = '#' then mat.(y).(x) <- "#"
          else
            let iy y = Util.between 0 y h in
            let ix x = Util.between 0 x w in
            mat.(y).(x) <-
              string_of_int @@ Bool.to_int(iy (y-1) && ix (x-1) && arr.(y-1).[x-1] = '#')
                               + Bool.to_int(iy (y-1) && arr.(y-1).[x] = '#')
                               + Bool.to_int(iy (y-1) && ix (x+1) && arr.(y-1).[x+1] = '#')
                               + Bool.to_int(ix (x-1) && arr.(y).[x-1] = '#')
                               + Bool.to_int(ix (x+1) && arr.(y).[x+1] = '#')
                               + Bool.to_int(iy (y+1) && ix (x-1) && arr.(y+1).[x-1] = '#')
                               + Bool.to_int(iy (y+1) && arr.(y+1).[x] = '#')
                               + Bool.to_int(iy (y+1) && ix (x+1) && arr.(y+1).[x+1] = '#')));
  Array.iter mat ~f:(fun a ->
      Array.iter a ~f:(Printf.printf "%s");
      Printf.printf "\n")
