...
module List = struct
  ...
  let rec lst = function
    | [] -> raise @@ Invalid_argument "list is null"
    | [x] -> x
    | _::ls -> lst ls
...
let () =
  let (n, k) = Scanf.scan "%d %d" Util.tuple2 in
  let arr = Scanf.lines n "%d" Fun.id
           |> List.sort ~cmp:Int.compare |> Array.of_list in
  let rec aux i m =
    if i + k - 1 >= n then m
    else
      aux (succ i) @@ min m (arr.(i+k-1) - arr.(i))
  in
  Printf.printf "%d\n" @@ aux 0 Int.max_int
